...
- 移动端设置页现已只保留后端 API 地址配置，默认地址改为内置安全值并隐藏显示；下游 FileBrowser/Mongo 配置不再由移动端编辑，也不再保存在移动端密钥存储中。
