...
- 保险、说明书、冰箱贴、备忘录表单改为“空白起步 + 必填提示”，避免误导性默认值
- 保险与说明书详情页会直接展示附件文件名、大小、最后同步时间与缺省状态
- 资料列表、提醒中心、搜索页支持页面内失败反馈与“手动重试”
- 首页模块按区域隔离加载，单模块失败不会拖垮整页摘要
- 后端会在启动时尝试初始化 Mongo 索引，为常见列表、详情与检索路径提供更稳定的查询基础
...
1. 新增或编辑保险、说明书、冰箱贴、备忘录时，直接保存应出现明确必填提示，且已输入内容会保留
2. 打开保险或说明书详情页，确认附件区块能展示文件名、大小、最后同步时间或明确缺省状态
3. 在资料列表、提醒中心、搜索页制造读取失败后，确认页面出现可见错误反馈和“手动重试”
4. 启动 `HomeHoney.Api` 后检查日志，确认 Mongo 索引初始化成功或被安全跳过
