````chatagent
---
description: 根据 `tasks.md` 处理并执行所有任务，推动实现计划落地。
---
...
2. **检查清单状态**（如果 FEATURE_DIR/checklists/ 存在）：
   ...
   - 计算总体状态：
     - **通过（PASS）**：所有清单的未完成项数为 0
     - **失败（FAIL）**：一个或多个清单存在未完成项

   - **如果任何清单未完成**：
     - 显示包含未完成项数的表格
     - **停止**并询问："部分清单尚未完成。是否仍要继续实施？（是/否）"
     - 等待用户回复后再继续
     - 如果用户回答"否"、"等等"或"停止"，则终止执行
     - 如果用户回答"是"、"继续"或"进行"，则继续执行步骤 3
...
4. **项目设置验证**：
   ...
   **检测与创建逻辑**：
   - 检查以下命令是否成功执行以确定仓库是否为 git 仓库（如果是则创建/验证 .gitignore）：

     ```sh
     git rev-parse --git-dir 2>/dev/null
     ```

   - 检查 Dockerfile* 是否存在或 plan.md 中是否包含 Docker → 创建/验证 .dockerignore
   - 检查 .eslintrc* 是否存在 → 创建/验证 .eslintignore
   - 检查 eslint.config.* 是否存在 → 确保配置的 `ignores` 条目覆盖所需模式
   - 检查 .prettierrc* 是否存在 → 创建/验证 .prettierignore
   - 检查 .npmrc 或 package.json 是否存在 → 创建/验证 .npmignore（如需发布）
...
注意：此命令假设 tasks.md 中已存在完整的任务分解。如果任务不完整或缺失，建议先运行 `/speckit.tasks` 重新生成任务列表。
